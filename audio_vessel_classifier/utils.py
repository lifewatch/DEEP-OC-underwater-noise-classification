...
def plot_confusion_matrix(
    ...
    )  # Calculate percentage of the entire matrix
    ...
    plt.xticks(
        tick_marks, classes, rotation=45, fontsize=16
    )  # Increased tick label font size
    plt.yticks(tick_marks, classes, fontsize=16)

    fmt = "d"  # Display raw counts
    thresh = cm_percentage.max() / 2.0  # Set threshold for text color
...
class DatasetEmbeddings(DataLoader):
    ...
    def __getitem__(self, idx):
        embedding_path = os.path.join(
            self.folder_path, self.desc, f"embedding_{idx}.pt"
        )
...
class DatasetWaveform(DataLoader):
    ...
    def __getitem__(self, idx):
        ...
        wav_path = row[
            "filename"
        ]  # list(self.wavs_folder.glob('**/' + row['filename']))[0]
        ...
        return (
            waveform,
            torch.tensor(self.label_to_id[row["label"]]),
            wav_path,
        )
...
def max_finder(logits, ids):
    ...
    for tensor in logits:
        max_index = torch.argmax(
            tensor
        ).item()  # Get the index of the maximum value in the tensor
...
def process_filenames(d_train):
    ...
    df["distance"] = df["filename"].apply(
        lambda x: float(x.split("_")[-1].split(".wav")[0])
    )

    # Extract speed from the filename
    df["speed"] = df["filename"].apply(
        lambda x: float(x.split("_")[-2].replace("-", "."))
    )
    df = df[df["speed"] <= 30]
